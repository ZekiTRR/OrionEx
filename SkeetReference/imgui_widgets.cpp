...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    const ImVec2 text_pos(window->DC.CursorPos.x + 20, window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec2 pos = window->DC.CursorPos + ImVec2(20, 0);
    ...
    // Render
    window->DrawList->AddRectFilledMultiColor( bb.Min, bb.Max, ( hovered && held ) ? to_vec4( 26, 26, 26, style.Alpha ) : hovered ? to_vec4( 40, 40, 40, style.Alpha ) : to_vec4( 34, 34, 34, style.Alpha ), ( hovered && held ) ? to_vec4( 26, 26, 26, style.Alpha ) : hovered ? to_vec4( 40, 40, 40, style.Alpha ) : to_vec4( 34, 34, 34, style.Alpha ), ( hovered && held ) ? to_vec4( 34, 34, 34, style.Alpha ) : hovered ? to_vec4( 32, 32, 32, style.Alpha ) : to_vec4( 26, 26, 26, style.Alpha ), ( hovered && held ) ? to_vec4( 34, 34, 34, style.Alpha ) : hovered ? to_vec4( 32, 32, 32, style.Alpha ) : to_vec4( 26, 26, 26, style.Alpha ) );
    window->DrawList->AddRect( bb.Min, bb.Max, GetColorU32( ImGuiCol_Border ) );
    window->DrawList->AddRect( bb.Min + ImVec2( 1, 1 ), bb.Max - ImVec2( 1, 1 ), ImColor( 1.f, 1.f, 1.f, 0.1f * style.Alpha ) );

    PushStyleColor( ImGuiCol_Text, ImVec4( ImColor( 0.f, 0.f, 0.f, 0.43f ) ) );
    RenderText( bb.GetCenter( ) - label_size / 2 + ImVec2( 1, 1 ), label );
    PopStyleColor( );
...
bool ImGui::Hotkey( const char* label, int* k ) {
    ...
    float w = GetWindowWidth( ) - 34;
    ImGuiID id = window->GetID( label );
    ImVec2 label_size = CalcTextSize( label, 0, 1 );

    ImRect frame_bb( window->DC.CursorPos + ImVec2( w - CalcTextSize( KeyNames[*k] ).x, 0 ), window->DC.CursorPos + ImVec2( w, CalcTextSize( KeyNames[*k] ).y ) );
    ImRect total_bb( window->DC.CursorPos + ImVec2( 20, 0 ), frame_bb.Max );
    ...
    value->second = ImLerp( value->second, ( IsItemActive( ) ? 1.f : 0.f ), 0.035f );

    // Render
    std::string buf_display = KeyNames[*k];
    RenderText( ImVec2( total_bb.Min.x, frame_bb.GetCenter( ).y - label_size.y / 2 ), label );

    for ( int i = -1; i < 2; ++i ) {

		for ( int j = -1; j < 2; ++j )
            window->DrawList->AddText( frame_bb.Min + ImVec2( i, j ), GetColorU32( ImGuiCol_Border, 0.5f ), buf_display.c_str( ) );
	}
    window->DrawList->AddText( frame_bb.Min, g.ActiveId == id ? to_vec4( 255, 0, 0, g.Style.Alpha ) : ( hovered ? to_vec4( 141, 141, 141, g.Style.Alpha ) : to_vec4( 114, 114, 114, g.Style.Alpha ) ), buf_display.c_str( ) );
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(bb_frame.Min, bb_frame.Max - ImVec2( 1, 0 ), bg_col, window->WindowRounding, flags);
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
        grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm), bb.Min.y, ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels, bb.Max.y);
    else
        grab_rect = ImRect(bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm), bb.Max.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) + grab_h_pixels);
    window->DrawList->AddRectFilled(grab_rect.Min - ImVec2(1, 0), grab_rect.Max, grab_col, style.ScrollbarRounding);
...
bool ImGui::Checkbox( const char* label, bool* v, bool special ) {
    ...
    const ImRect total_bb( pos, pos + ImVec2( square_sz + label_size.x + 10, square_sz ) );
    ...
    window->DrawList->AddRectFilledMultiColor( frame_bb.Min, frame_bb.Max, hovered ? to_vec4( 91, 91, 91, style.Alpha ) : to_vec4( 76, 76, 76, style.Alpha ), hovered ? to_vec4( 91, 91, 91, style.Alpha ) : to_vec4( 76, 76, 76, style.Alpha ), hovered ? to_vec4( 66, 66, 66, style.Alpha ) : to_vec4( 51, 51, 51, style.Alpha ), hovered ? to_vec4( 66, 66, 66, style.Alpha ) : to_vec4( 51, 51, 51, style.Alpha ) );
    if ( *v )
        window->DrawList->AddRectFilledMultiColor( frame_bb.Min, frame_bb.Max, custom.get_accent_color( style.Alpha ), custom.get_accent_color( style.Alpha ), custom.get_accent_color( 0.6f * style.Alpha ), custom.get_accent_color( 0.6f * style.Alpha ) );

    window->DrawList->AddRect( frame_bb.Min, frame_bb.Max, GetColorU32( ImGuiCol_Border ), ROUNDING );

    ImVec2 label_pos = ImVec2( pos.x + 20, frame_bb.GetCenter( ).y - label_size.y / 2 );
    if ( special )
        PushStyleColor( ImGuiCol_Text, ImVec4( 182 / 255.f, 182 / 255.f, 101 / 255.f, 1.f ) );
    if ( label_size.x > 0.f ) 
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const float size = GetWindowWidth( ) - 50; // GetWindowWidth( ) - 34

    auto label_size = CalcTextSize( label, 0, 1 );
    const ImRect clickable( window->DC.CursorPos + ImVec2( 20, label_size.y + 4 ), window->DC.CursorPos + ImVec2( size - 20, label_size.y + 25 ) );
    ...
    window->DrawList->AddRectFilledMultiColor( clickable.Min, clickable.Max, hovered || popup_open ? to_vec4( 41, 41, 41, style.Alpha ) : to_vec4( 31, 31, 31, style.Alpha ), hovered || popup_open ? to_vec4( 41, 41, 41, style.Alpha ) : to_vec4( 31, 31, 31, style.Alpha ), hovered || popup_open ? to_vec4( 46, 46, 46, style.Alpha ) : to_vec4( 36, 36, 36, style.Alpha ), hovered || popup_open ? to_vec4( 46, 46, 46, style.Alpha ) : to_vec4( 36, 36, 36, style.Alpha ) );
    window->DrawList->AddRect( clickable.Min, clickable.Max, GetColorU32( ImGuiCol_Border ) );

    RenderText( ImVec2( clickable.Min.x, total_bb.Min.y ), label );

    PushStyleColor( ImGuiCol_Text, ImVec4( 0.64f, 0.64f, 0.64f, 1.f ) );
    RenderTextClipped( clickable.Min + ImVec2( 9, 4 ), clickable.Max - ImVec2( 16, 0 ), preview_value, NULL, NULL, ImVec2( 0.f, 0.f ) );
    PopStyleColor( );

    RenderArrow( window->DrawList, ImVec2( clickable.Max.x - 14, clickable.Min.y + 7 ), GetColorU32( ImGuiCol_Text, 0.65f ), ImGuiDir_Down, 0.8f );
...
bool ImGui::SliderScalar( const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags ) {
    ...
    const ImVec2 label_size = CalcTextSize( label, 0, 1 );
    const ImRect frame_bb( window->DC.CursorPos + ImVec2( 20, label_size.y + 4 ), window->DC.CursorPos + ImVec2( w - 20, label_size.y + 10 ) );
    ...
    window->DrawList->AddRectFilledMultiColor( frame_bb.Min, frame_bb.Max, ( hovered || IsItemActive( ) ) ? to_vec4( 91, 91, 91, style.Alpha ) : to_vec4( 76, 76, 76, style.Alpha ), ( hovered || IsItemActive( ) ) ? to_vec4( 91, 91, 91, style.Alpha ) : to_vec4( 76, 76, 76, style.Alpha ), ( hovered || IsItemActive( ) ) ? to_vec4( 66, 66, 66, style.Alpha ) : to_vec4( 51, 51, 51, style.Alpha ), ( hovered || IsItemActive( ) ) ? to_vec4( 66, 66, 66, style.Alpha ) : to_vec4( 51, 51, 51, style.Alpha ) );
    window->DrawList->AddRectFilledMultiColor( frame_bb.Min, ImVec2( grab_bb.Max.x + 1, frame_bb.Max.y ), custom.get_accent_color( style.Alpha ), custom.get_accent_color( style.Alpha ), custom.get_accent_color( 0.6f * style.Alpha ), custom.get_accent_color( 0.6f * style.Alpha ) );
    window->DrawList->AddRect( frame_bb.Min, frame_bb.Max, GetColorU32( ImGuiCol_Border ) );
    ...
    for ( int i = -1; i < 2; ++i ) {
		...
		for ( int j = -1; j < 2; ++j )
            window->DrawList->AddText( GetIO( ).Fonts->Fonts[1], GetIO( ).Fonts->Fonts[1]->FontSize, grab_bb.Max - ImVec2( 6, 4 ) + ImVec2( i, j ), GetColorU32( ImGuiCol_Border, 0.5f ), value_buf, value_buf_end );
	}
    window->DrawList->AddText( GetIO( ).Fonts->Fonts[1], GetIO( ).Fonts->Fonts[1]->FontSize, grab_bb.Max - ImVec2( 6, 4 ), GetColorU32( ImGuiCol_Text ), value_buf, value_buf_end );
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ...
    return InputTextEx(label, NULL, buf, (int)buf_size, ImVec2(GetWindowWidth() - 90, 0), flags, callback, user_data);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(20, label_size.y + 4), window->DC.CursorPos + frame_size + ImVec2(20, label_size.y + 4));
    ...
    if (!is_multiline)
    {
        window->DrawList->AddRectFilledMultiColor( frame_bb.Min, frame_bb.Max, hovered || IsItemActive( ) ? to_vec4( 41, 41, 41, style.Alpha ) : to_vec4( 31, 31, 31, style.Alpha ), hovered || IsItemActive( ) ? to_vec4( 41, 41, 41, style.Alpha ) : to_vec4( 31, 31, 31, style.Alpha ), hovered || IsItemActive( ) ? to_vec4( 46, 46, 46, style.Alpha ) : to_vec4( 36, 36, 36, style.Alpha ), hovered || IsItemActive( ) ? to_vec4( 46, 46, 46, style.Alpha ) : to_vec4( 36, 36, 36, style.Alpha ) );
        window->DrawList->AddRect( frame_bb.Min, frame_bb.Max, GetColorU32( ImGuiCol_Border ) );
    }
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags, ImVec2(20, 10)))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                ...
                OpenPopup("picker");
                SetNextWindowPos(g.LastItemData.Rect.GetBL() + ImVec2(0.0f, style.ItemSpacing.y / 2));
            }
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
            OpenPopupOnItemClick("context", ImGuiPopupFlags_MouseButtonRight);
        ...
        if (BeginPopup("picker", ImGuiWindowFlags_NoMove))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ...
                SetNextItemWidth(square_sz * 9.4f); // Use 256 + bar sizes?
...
{
    draw_list->AddRectFilled( pos, pos + ImVec2( bar_w, 4 ), ImColor( 1.f, 1.f, 1.f, 0.4f * alpha ) );
    draw_list->AddRect( pos, pos + ImVec2( bar_w, 4 ), ImGui::GetColorU32( ImGuiCol_Border ) );
}
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        draw_list->AddRect( picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), GetColorU32( ImGuiCol_Border ) );
        ...
        draw_list->AddRect( ImVec2( bar0_pos_x, picker_pos.y ), ImVec2( bar0_pos_x + bars_width, picker_pos.y + sv_picker_size ), GetColorU32( ImGuiCol_Border ) );
        RenderArrowsForVerticalBar(draw_list, ImVec2(bar0_pos_x, bar0_line_y), ImVec2(bars_triangles_half_sz, bars_triangles_half_sz), bars_width, style.Alpha);
    }

    // Render cursor/preview circle (clamp S/V within 0..1 range because floating points colors may lead HSV values to be out of range)
    float sv_cursor_rad = 4;
    draw_list->AddRectFilled( sv_cursor_pos - ImVec2( sv_cursor_rad / 2, sv_cursor_rad / 2 ), sv_cursor_pos + ImVec2( sv_cursor_rad / 2, sv_cursor_rad / 2 ), ImColor( 1.f, 1.f, 1.f, 0.4f * style.Alpha ) );
    draw_list->AddRect( sv_cursor_pos - ImVec2( sv_cursor_rad / 2, sv_cursor_rad / 2 ), sv_cursor_pos + ImVec2( sv_cursor_rad / 2, sv_cursor_rad / 2 ), GetColorU32( ImGuiCol_Border ) );
    ...
    if (alpha_bar)
    {
        ...
        draw_list->AddRectFilled(bar1_bb.Min, bar1_bb.Max, ImColor(col[0], col[1], col[2], style.Alpha));
        draw_list->AddRect(bar1_bb.Min, bar1_bb.Max, GetColorU32(ImGuiCol_Border));
        float bar1_line_x = IM_ROUND(picker_pos.x + (1.0f - alpha) * sv_picker_size);
        custom.render_arrows_for_horizontal_bar(ImVec2(bar1_line_x - 1, bar1_bb.Min.y), style.Alpha, 4, bars_width);
    }
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + size);
    const ImRect bb_s(window->DC.CursorPos, window->DC.CursorPos);
    ItemSize(bb_s, g.Style.FramePadding.y);
    if (!ItemAdd(bb, id))
        return false;
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (hovered || selected)
    {
        ImColor col = to_vec4( 23, 22, 23, GetStyle( ).Alpha );
    ...
    if ( selected )
        PushStyleColor( ImGuiCol_Text, ImVec4( custom.get_accent_color( ) ) );
