...
(async function initBridge() {
    if (window.chrome && window.chrome.webview) {
        ...
        await window.chrome.webview.hostObjects.bridge;

        window.bridge = window.chrome.webview.hostObjects.bridge;
...
var bridge = (window.chrome && window.chrome.webview) ? window.chrome.webview.hostObjects.bridge : null;
...
function navigateTo(page) {
    ...
    if (page === 'console' && !settings.errorSpoofing && !settings.skipSpoofWarning && !skipSpoofWarningSession && !window.isTutorialRunning) {
...
window.addEventListener('DOMContentLoaded', function () {
    ...
    if (btnExec) {
        btnExec.addEventListener('click', function () {
            var statusText = document.getElementById('status-text').innerText.toLowerCase();
            if (statusText === 'inactive' || statusText === 'not injected' || statusText === 'injecting...') {
                if (window.showNotification) window.showNotification("Failed to execute - SirHurt is not injected");
                return;
            }
            if (isSplitView) {
